...
import { Button, GlassCard, Input, TextArea, Badge } from './UI';
import { ProjectWizardState, ProjectCategory, TimeframeOption, Currency } from '../types';
...
export const NewProjectWizard: React.FC<NewProjectWizardProps> = ({ onClose, onProjectCreated, currency, t }) => {
  ...
  useEffect(() => {
    const price = ApiService.calculateEstimate(state);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await ApiService.submitProjectRequest(state);
      onProjectCreated();
      onClose();
    } catch (error) {
      console.error(error);
  ...
  const renderQuestions = () => {
    ...
    if (state.serviceType === 'stereo_mix') {
      return (
        <div className="space-y-4">
          <SelectField label="Duration" value={state.answers['duration']} onChange={(v) => updateAnswer('duration', v)} options={[
            { val: 'lt_5', txt: '< 5 Minutes' },
            { val: '5_11', txt: '5 - 11 Minutes' },
            { val: 'gt_11', txt: '> 11 Minutes' }
          ]} />
          <SelectField label="Track Count" value={state.answers['tracks']} onChange={(v) => updateAnswer('tracks', v)} options={[
    ...
    if (state.serviceType === 'media_mix') {
      return (
        <div className="space-y-4">
          <SelectField label="Content Type" value={state.answers['content_type']} onChange={(v) => updateAnswer('content_type', v)} options={[
            { val: 'short', txt: 'Short Film / Ad (< 15 min)' },
            { val: 'medium', txt: 'Mid-length (15 - 45 min)' },
            { val: 'feature_film', txt: 'Feature Film (> 45 min)' }
          ]} />
          <SelectField label="Delivery Format" value={state.answers['format']} onChange={(v) => updateAnswer('format', v)} options={[
    ...
    if (state.serviceType === 'podcast_edit') {
       return (
        <div className="space-y-4">
          <SelectField label="Raw Material Duration" value={state.answers['raw_duration']} onChange={(v) => updateAnswer('raw_duration', v)} options={[
            { val: 'lt_30', txt: '< 30 Minutes' },
            { val: '30_60', txt: '30 - 60 Minutes' },
            { val: 'gt_60', txt: '> 60 Minutes' }
          ]} />
          <SelectField label="Number of Voices" value={state.answers['voices']} onChange={(v) => updateAnswer('voices', v)} options={[
            { val: '1', txt: '1 Voice' },
            { val: '2', txt: '2 Voices' },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <GlassCard className="w-full max-w-2xl relative z-10 max-h-[90vh] overflow-y-auto flex flex-col">
        {/* Header */}
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/10">
           ...
           <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-[10px] uppercase tracking-wider opacity-60">Estimate</p>
                <p className="text-lg font-bold text-j-light-text dark:text-white">
                  {ApiService.formatCurrency(state.estimatedPrice, currency)}
                </p>
              </div>
        ...
        <div className="flex-1 min-h-[300px]">
           <AnimatePresence mode="wait">
             ...
             >
               ...
               {currentStep === 4 && (
                 <div className="space-y-6">
                    <div className="bg-black/20 p-4 rounded-xl space-y-2 text-sm text-j-light-text/80 dark:text-j-dark-text/80 border border-white/5">
                       <p><strong className="text-white">Category:</strong> {state.category}</p>
                       <p><strong className="text-white">Service:</strong> {state.serviceType}</p>
                       <p><strong className="text-white">Timeframe:</strong> {state.timeframe}</p>
                       <p><strong className="text-white">Context:</strong> {state.context || 'N/A'}</p>
                    </div>
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group">
                       <input 
                         ...
                         className="mt-1 w-5 h-5 rounded border-white/20 bg-black/20 text-warm-glow focus:ring-warm-glow"
                       />
...
const SelectField = ({ label, value, onChange, options }: any) => (
  <div className="space-y-1">
    <label className="text-xs font-medium text-white/60 uppercase tracking-wider ml-1">{label}</label>
      ...
      {options.map((opt: any) => (
        <button
          ...
          className={`px-4 py-3 rounded-lg text-sm text-left border transition-all ${
            value === opt.val
            ? 'bg-warm-glow/20 border-warm-glow text-white'

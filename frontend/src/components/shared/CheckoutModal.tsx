...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const formattedAmount = formatCurrency(finalAmount, currency);
  const formattedTotal = formatCurrency(basePrice, currency);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/orders/create', {
        ...
        body: JSON.stringify({ serviceId: service.id, currency }),
      });
      ...
      if (project) {
        await fetch(`/api/projects/${project.id}/payment`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-j-light-text/60 dark:text-j-dark-text/60 text-sm mt-1">
                      {project ? (typeof service.name === 'string' ? service.name : service.name['en'] || service.name) : t.checkout.professionalAudio}
                    </p>
                  </div>
                ...
                {project && (
                  <div className="bg-black/10 dark:bg-white/5 p-1 rounded-lg grid grid-cols-2 gap-1">
                    <button
                      ...
                      className={`py-2 px-4 rounded-md text-sm font-medium transition-all ${paymentType === 'FULL'
                    ...
                    <button
                      ...
                      className={`py-2 px-4 rounded-md text-sm font-medium transition-all ${paymentType === 'SPLIT'
                        ? 'bg-white text-black shadow-sm'
                        : 'text-j-light-text/60 dark:text-white/60 hover:text-white'
                        }`}
                    >
